...
![image](https://github.com/user-attachments/assets/b53e9cf5-c27d-42fb-927c-69aaf17dc39f)

**Use case диаграмма**
### **Описание диаграммы информационной системы склада**  

Диаграмма отображает **взаимодействие пользователей и внешних систем** с **информационной системой склада** (ИС Склада).  

#### **Ключевые элементы:**  
1. **Акторы (роли):**  
   - **Кладовщик** – выполняет операции с товарами (добавление, перемещение, инвентаризация).  
   - **Менеджер** – управляет зонами хранения и формирует отчеты.  
   - **Бухгалтерская система** – автоматически получает данные о движении товаров для учета.  

2. **Основные функции (Use Case):**  
   - **Управление товарами** (добавление, перемещение).  
   - **Контроль склада** (инвентаризация, добавление зон хранения).  
   - **Отчетность** (анализ остатков, распределение по зонам).  
   - **Интеграция с бухгалтерией** (передача данных о приходе/расходе).  

3. **Связи:**  
   - Кладовщик и менеджер работают внутри системы.  
   - Бухгалтерская система подключена для автоматического обмена данными.  

#### **Цель системы:**  
Автоматизировать **учет товаров**, **оптимизировать хранение** и **упростить отчетность**, синхронизируясь с бухгалтерским ПО.  

Диаграмма помогает наглядно представить:  
- Кто и какие функции использует.  
- Как данные передаются между компонентами.  
- Где требуются интеграции с внешними сервисами.
